...
export default Nav = ({ dimensions, currentView, changeView }) => {
  return (
    <Header style={{ height: dimensions.height, flexDirection: "row", justifyContent: 'space-between' }}
      ...
      rightComponent={<Button icon={{ name: 'home', size: ICON_STYLES.size, color: ICON_STYLES.color }} buttonStyle={{ backgroundColor: '#FFF' }} onPress={() => {
        if (currentView !== 'MovieSwipeDeck') {
